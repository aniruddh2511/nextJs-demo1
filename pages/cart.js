...
const Cart = ({error,products})=>{
        ...
        const [cProduct,setCartProduct] = useState(products);
        let price = 0;
                  ...
                  price = price + item.quantity * item.product.price
                    return(
                        <div style={{display:"flex", margin:"20px"}} key={item._id}>
                            <img src={item.product.mediaUrl} style={{width:"200px"}} />
                            <div style={{marginLeft:"20px"}}>
                                    <h4>{item.product.name}</h4>
                                     <h4>{item.quantity} x {item.product.price}</h4>
    ...
    const handleChekout = async (paymentInfo) =>{
    ...
    const TotalPrice = ()=>{
        return(
            <div className="container" style={{display:"flex",justifyContent:"space-between"}}>
               ...
               billingAddress={true}
               zipCode={true}
               stripeKey="pk_test_Vn7fR47sMfDYk0QErLnqPQJu"
               token={(paymentInfo)=>handleChekout(paymentInfo)}
